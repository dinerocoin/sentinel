...
class GovernanceObject(BaseModel):
    ...
    # sync dinerod gobject list with our local relational DB backend
    @classmethod
    def sync(self, dinerod):
        golist = dinerod.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_dinerod(dinerod, item)
    ...
    @classmethod
    def import_gobject_from_dinerod(self, dinerod, rec):
        import decimal
        import dinerolib
        import inflection
        ...
        # shim/dinerod conversion
        object_hex = dinerolib.SHIM_deserialise_from_dinerod(object_hex)
        objects = dinerolib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from dinerod...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from dinerod, with every run
        ...
        # get/create, then sync payment amounts, etc. from dinerod - Dinerod is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(dinerod)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from dinerod! %s" % e)
            govobj.vote_delete(dinerod)
            return (govobj, None)
    ...
    def vote_delete(self, dinerod):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(dinerod, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, dinerod, signal, outcome):
        import dinerolib
        ...
        output = dinerod.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = dinerolib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(dinerod, signal)

    def sync_network_vote(self, dinerod, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = dinerod.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = DINEROD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 dinero addr, non-multisig
            if not dinerolib.is_valid_dinero_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Dinero address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (dinerolib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/DineroDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import dinerolib
        obj_data = dinerolib.SHIM_serialise_for_dinerod(self.serialise())
    ...
    def prepare(self, dinerod):
        try:
            object_hash = dinerod.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = DINEROD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not dinerolib.is_valid_dinero_address(addr, config.network):
    ...
    def hash(self):
        import dinerolib
        return dinerolib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = DINEROD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, dinerod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - dinerod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, dinerod):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - dinerod.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, dinerod):
        now = int(time.time())
        return (self.created_at < (now - dinerod.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, dinerod):
        if self.is_expired(dinerod):
            return False

        return True

    def is_deletable(self, dinerod):
        if self.is_expired(dinerod):
            return True
