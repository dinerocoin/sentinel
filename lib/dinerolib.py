...
def is_valid_dinero_address(address, network='mainnet'):
    ...
    dinero_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Dinero address
        return False

    if (address_version != dinero_version):
        return False
...
# shims 'til we can fix the dinerod side
def SHIM_serialise_for_dinerod(sentinel_hex):
    from models import DINEROD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = DINEROD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in dinerod
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # dinerod expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    dinerod_hex = serialise(obj)
    return dinerod_hex


# shims 'til we can fix the dinerod side
def SHIM_deserialise_from_dinerod(dinerod_hex):
    from models import DINEROD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(dinerod_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('dinero.conf')
