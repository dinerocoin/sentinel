...
class DineroDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some dinerod methods
    ...
    @classmethod
    def from_dinero_conf(self, dinero_dot_conf):
        from dinero_config import DineroConfig
        config_text = DineroConfig.slurp_config_file(dinero_dot_conf)
        creds = DineroConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = dinerolib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = dinerolib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = dinerolib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_dinerod_proto_version_with_sentinel_ping)
