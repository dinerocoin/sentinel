...
def has_dinero_conf():
    ...
    valid_dinero_conf = False

    # ensure dinero_conf exists & readable
    #
    # if not, print a message stating that Dinero Core must be installed and
    # configured, including JSONRPC access in dinero.conf
    try:
        f = io.open(config.dinero_conf)
        valid_dinero_conf = True
    except IOError as e:
        print(e)

    return valid_dinero_conf
...
def main():
    ...
    if not has_dinero_conf():
        print("DineroCore must be installed and configured, including JSONRPC access in dinero.conf")
        sys.exit(1)
